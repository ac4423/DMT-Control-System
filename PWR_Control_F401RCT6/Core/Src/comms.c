...
extern volatile uint16_t heartbeat_period_ms;
...
#if SKIP_CRC_CHECK
                // Always accept message regardless of CRC
                last_good_rx_tick = SYSTEM_TICK;

                switch (tmp_type) {
#else
                if (crc_calc == b) {  // CRC matches
                    last_good_rx_tick = SYSTEM_TICK;
                    ...
                    switch (tmp_type) {
                        ...
                        case MSG_HANDSHAKE:
                            /* Expected minimal payload:
                                ...
                                [send_ack (1 byte)]
                               Total 5 bytes minimum. Extra fields ignored.
                            */
                            if (tmp_len >= 5) {
                        ...
                        default:
#if !SKIP_CRC_CHECK
                            // Only send NACK if CRC was checked and failed
                            if (send_ack_and_nack_packets) Comms_SendNack(tmp_seq);
#endif
                            break;
                    } // switch tmp_type
